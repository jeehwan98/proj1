...
AWS infrastructure for application — Next.js frontend, Spring Boot backend, and Lambda email sender, orchestrated with ECS Fargate.
...
| `security_groups.tf` | Security groups for ALB, ECS, RDS, Lambda |
...
| `secrets.tf` | Secrets Manager for DB password, SMTP, Google SA |
| `sqs.tf` | Invoice email queue + dead letter queue |
| `lambda.tf` | Lambda function, IAM role, SQS trigger |
| `outputs.tf` | ALB DNS, ECR URLs, RDS endpoint, SQS URL |
